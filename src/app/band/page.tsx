        ...
        {members.map((member, index) => (
          ...
          >
            ...
            <div className="flex items-center">
              ...
              {/* The toggle button can “float” to the right by pushing it with margin-left: auto */}
              <div className="justify-center item-center ml-8">
                <button
                  onClick={() => handleToggle(member.id)}
                  className={`py-2 px-4 rounded-full text-white font-semibold ${getToggleStyles(
                    member.instrument
                  )}`}
                >
                  {activeToggle.includes(member.id) ? "Hide Bio" : "Show Bio"}
                </button>
              </div>
            </div>
